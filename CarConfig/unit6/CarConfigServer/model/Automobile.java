...
public class Automobile implements Serializable {
	...
	private final String INSERT_MODEL = "insert into Models values(\"%s\",\"%s\",%f);";
	private final String INSERT_OPSET = "insert into OptionSet(name, model) values(\"%s\",\"%s\");";
	private final String INSERT_OPTION = "insert into Options(name, price, opsetid) "
			+ "values(\"%s\",%f,(select id from OptionSet where model=\"%s\" and name=\"%s\"));";
	...
	public boolean updateOptionSetName(String optionSetName, String newName) {
		try {
			...
			System.out.println("OptionSet " + optionSetName
					+ " does not exist.");
			return false;
		}
	}
	...
	public boolean updateOptionPrice(String optionSetName, String optionName,
			float newPrice) {
		try {
			findOpsetByName(optionSetName).updateOptionPrice(optionName,
					newPrice);
			return true;
		} catch (NullPointerException e) {
			return false;
	...
	public void writeAutoToDB(JDBCAdapter adapter) {
		if (adapter == null) {
			return;
		}
		adapter.executeWriteQuery(String
				.format(INSERT_MODEL, model, make, basePrice));
		
		for (OptionSet opset : opsets) {
			adapter.executeWriteQuery(String.format(INSERT_OPSET, opset.getName(),
					model));
			for (Option option : opset.getOptions()) {
				adapter.executeWriteQuery(String.format(INSERT_OPTION,
						option.getName(), option.getPrice(), model, opset.getName()));
			}
		}
	}
